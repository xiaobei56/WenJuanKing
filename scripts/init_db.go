...
func main() {
	...
	if connStr == "" {
		connStr = "host=localhost port=5432 user=postgres password=postgres dbname=surveyking sslmode=disable"
	}
...
CREATE INDEX IF NOT EXISTS idx_questions_order ON questions(project_id, order_num);
...
CREATE INDEX IF NOT EXISTS idx_answer_details_answer_id ON answer_details(answer_id);
CREATE INDEX IF NOT EXISTS idx_answer_details_question_id ON answer_details(question_id);
